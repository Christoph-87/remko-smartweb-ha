...
| KWT | `KwtDeviceProfile` | values and KWT ESP direct readback | climate value IDs; direct KWT ESP before fallback | Partial: KWT direct value write tests | no local-specific override yet |
...
| WPM | `WpmDeviceProfile` | values | numbers/switches value IDs | Profile encoding covered; entity write path not fully contract-tested | no local-specific override yet |
...
|---|---|---|---|---|
| `climate.set_hvac_mode` | Generic AC/KWT | C0 `/ESP` if C0 available; value-write fallback | `/RESP` / values | Good for Generic AC; KWT entity path needs explicit contract |
| `climate.set_temperature` | Generic AC/KWT | same as climate mode | `/RESP` / values | Good for Generic AC; KWT entity path needs explicit contract |
| `climate.set_fan_mode` | Generic AC/KWT | same as climate mode | `/RESP` / values | Needs explicit contract for both Generic AC cloud/local and KWT |
| `climate.set_swing_mode` | Generic AC/KWT | same as climate mode | `/RESP` / values | Needs explicit contract |
...
| `switch.turbo/sleep/eco/frost/bioclean` | Generic AC | C0 `/ESP` | `/RESP` | Covered for turbo only; add one test that all C0 extended switches route via C0 |
| `switch.beep` | all | local option only, no device write | n/a | Covered |
| WPM switches | WPM | value-write | values | Needs entity contract test |
| `number.*` | LTE/WPM/etc. | `set_value_ids(build_value_write)` | values | Needs entity contract tests |
...
2. Cloud contract: all Generic AC extended switches use C0 and do not value-write.
3. Cloud contract: KWT climate service methods write expected value IDs or ESP path unchanged from `main`.
4. Entity contract: LTE number/power writes expected value IDs.
5. Entity contract: WPM number/switch writes expected value IDs.
